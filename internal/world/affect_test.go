...
func TestAffectStrippedOnRespawnPredicate(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if got := affectStrippedOnRespawn(def, harmPolarity{}); got != tc.want {
	...
	if !affectStrippedOnRespawn(nil, harmPolarity{}) {
		t.Error("affectStrippedOnRespawn(nil, harmPolarity{}) should be true (fail toward stripping)")
	}
}
