...
func (z *Zone) leave(id string) {
	...
	if z.saver != nil && z.saver.enabled() && s.entity != nil {
		if s.entity.pid == nil {
			...
			// enqueueSave path cannot run yet (no PID to CAS on), and the in-flight createdMsg, when it
...
func (z *Zone) transferIn(m transferInMsg) {
	...
	if s.entity.living != nil {
		// Clear transferred combat state. For players (prototype==nil), mutableLiving is a
		// no-fork pass-through and still uses the same audited mutation path.
