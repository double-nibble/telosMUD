...
// with an OnRest handler gains its bump when the player rests, and a subsequent tick does NOT re-fire it.
func TestRestFiresOnRestOnce(t *testing.T) {
	...
	if got := resourceCurrent(caster.entity, "vigor"); got != 20 {
		t.Fatalf("vigor after rest = %d, want 20 (OnRest fired once on enter)", got)
	}
