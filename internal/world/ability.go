...
func (z *Zone) castAbility(s *session, def *abilityDef, arg string, rng *rand.Rand) {
	actor := s.entity
	// arg is the verbatim tail of the cast line (player input); attach it only under the opt-in (#454).
	if z.logRawInput {
		z.log.Debug("ability lifecycle: invoke", "ability", def.ref, "actor", actor.short, "arg", arg) // logkey-ok: gated by TELOS_LOG_RAW_INPUT (#454)
	} else {
		z.log.Debug("ability lifecycle: invoke", "ability", def.ref, "actor", actor.short)
	}
