...
func mainLoopWithContext(L *LState, baseframe *callFrame) {
	...
	for {
		...
		inst = cf.Fn.Proto.Code[cf.Pc]
		cf.Pc++
		// TELOSMUD FORK (docs/PHASE7-PLAN.md P7-D6 / T3): per-call instruction-count
		// abort, beside the existing ctx.Done() deadline. Keep this block in sync with
		// vm.go (this _vm.go template is the go-inline source). Disabled (zero-cost)
		// when instrBudget==0, so a VM that never opts in is byte-identical to upstream.
		if L.instrBudget > 0 {
			L.instrCount++
