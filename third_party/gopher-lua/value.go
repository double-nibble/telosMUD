...
type LState struct {
	...
	// TELOSMUD FORK (docs/PHASE7-PLAN.md P7-D6 / T3): a deterministic per-call
	// instruction-count abort. Upstream gopher-lua v1.1.1 has no SetHook/MaskCount/
	// debug-hook, so the count is enforced inline in mainLoopWithContext beside the
	// existing ctx.Done() select. instrBudget is the max VM instructions a single
	// entry-point call may execute (0 disables the check — the upstream default, so a
	// VM that never opts in behaves exactly like stock gopher-lua); instrCount is the
	// running tally, reset at the same per-call chokepoint as SetContext. See
	// SetInstructionBudget / ResetInstructionCount below.
	instrBudget int
	instrCount  int
}

// SetInstructionBudget arms (or, with n<=0, disarms) the per-call VM-instruction
// abort for this LState. TELOSMUD FORK (docs/PHASE7-PLAN.md P7-D6 layer 1). The
// counter increments once per executed bytecode instruction inside
// mainLoopWithContext and, past n, raises a Lua error ("instruction budget
// exceeded") on the same path the context deadline uses — caught by the engine's
// pcall. The budget only takes effect while a context is set (mainLoopWithContext is
// the active loop), which the engine's per-call chokepoint guarantees. Call
// ResetInstructionCount at the start of each entry-point call.
func (ls *LState) SetInstructionBudget(n int) {
	ls.instrBudget = n
}

// ResetInstructionCount zeroes the running instruction tally. TELOSMUD FORK. The
// engine calls this at the same chokepoint where it arms a fresh context deadline,
// so each entry-point call gets the full budget (the count must not carry across
// independent calls).
func (ls *LState) ResetInstructionCount() {
	ls.instrCount = 0
}

// InstructionCount returns the instructions executed since the last
// ResetInstructionCount. TELOSMUD FORK — exposed for tests/metrics.
func (ls *LState) InstructionCount() int {
	return ls.instrCount
}
