...
func mainLoopWithContext(L *LState, baseframe *callFrame) {
	...
	for {
		...
		// abort, beside the existing ctx.Done() deadline. Deterministic and
		// test-reproducible (a pure-CPU loop trips it identically every run, unlike
		// wall-clock). Disabled (zero-cost branch) when instrBudget==0, so a VM that
		// never opts in is byte-identical to upstream. Aborts via RaiseError, the same
		// path the deadline uses, so the engine's pcall catches it.
		if L.instrBudget > 0 {
			L.instrCount++
